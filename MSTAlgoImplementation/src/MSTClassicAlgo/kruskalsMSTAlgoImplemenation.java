...
public class kruskalsMSTAlgoImplemenation {
...
private static int Sum;
 ...
 System.out.println("Graph has "+ nodes +" vertex "+numEdges +" edges");
 ...
 */
 ...
 /* tree display */
 System.out.println("Kruskal MST implemenation");
 for (int u = 1; u < mst.length; u++) {
 for (int v = 0; v < mst.length; v++) {
 if (mst[u][v] != 0) {
	Sum=Sum+mst[u][v]; 
 System.out.println("Vertex "+u + " -->" + v + " has weight" + mst[u][v]);
 }
 }
 }
 System.out.println("total Sum of weight in Kruskal MST = "+Sum);
